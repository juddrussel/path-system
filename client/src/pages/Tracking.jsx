        ...
        <main className="path-tracking-content">
          ...
          <section className="tracking-layout">
            <article className="tracking-table tracking-panel">
              ...
              <div className="tracking-table-head"><span>Document</span><span>Assigned to</span><span>Assigned by</span><span>Stage</span><span>SLA</span></div>
              <div className="tracking-records">{loading ? <div className="tracking-empty"><Icon.Refresh /><strong>Loading tracking records</strong><span>Syncing the latest document handoffs.</span></div> : paginated.length === 0 ? <div className="tracking-empty"><Icon.Tracking /><strong>No documents match these filters</strong><span>Try another search or lifecycle status.</span></div> : paginated.map((doc) => <button key={doc.id || doc.document_id} type="button" className={"tracking-record " + (selected?.id === doc.id ? "selected" : "")} onClick={() => setSelected(doc)}><div className="tracking-document"><div><span className="tracking-record-id">{doc.document_id}</span><strong>{doc.title}</strong><span>{doc.department}{doc.source_type ? " · " + doc.source_type : ""}</span></div></div><div className="tracking-assignee"><Avatar name={doc.faculty_name || doc.current_handler || "Faculty"} size={28} color="#7c3aed" /><span>{doc.faculty_name || doc.current_handler || "Faculty"}</span></div><div className="tracking-assignee"><Avatar name={doc.assigned_by_name || doc.assigned_by || "Program Chair"} size={28} color="#a78bfa" /><span>{doc.assigned_by_name || doc.assigned_by || "Program Chair"}</span></div><div className="tracking-stage"><StatusBadge status={doc.status} /><span>{doc.stage || "Current handoff"}</span></div><div className={"tracking-sla " + (["returned", "rejected"].includes(doc.status?.toLowerCase()) ? "risk" : ["approved", "archived"].includes(doc.status?.toLowerCase()) ? "closed" : "")}><i />{["returned", "rejected"].includes(doc.status?.toLowerCase()) ? "Needs attention" : ["approved", "archived"].includes(doc.status?.toLowerCase()) ? "Closed" : "On track"}</div></button>)}</div>
            ...
            <aside className="tracking-detail tracking-panel"><div className="tracking-panel-topline"><div><div className="section-kicker">Selected record</div><h3>Current handoff</h3></div><button className="icon-button compact" type="button" onClick={() => selected && setSelected(selected)} aria-label="Refresh selected document"><Icon.Refresh /></button></div>{selected ? <><div className="tracking-detail-title"><span className="tracking-detail-avatar">{String(selected.submitted_by || selected.current_handler || "PATH").split(/\s+/).map((word) => word[0]).join("").slice(0, 2).toUpperCase()}</span><div><strong>{selected.title}</strong><span>{selected.document_id} · {selected.current_handler || "Program Chair"}</span></div></div><div className="tracking-detail-status-card"><span className="section-kicker">Current status</span><strong>{selected.status}</strong><span>{selected.status?.toLowerCase() === "approved" ? "Closed on time" : selected.stage || "Current handoff"}{selected.submitted_at ? " · due " + new Date(selected.submitted_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : ""}</span></div><div className="tracking-mini-timeline">{["Submitted", "Screening", "Review", "Decision"].map((stage, index) => { const currentStage = Math.max(0, ["submitted", "screening", "review", "decision"].findIndex((value) => String(selected.stage || selected.status || "").toLowerCase().includes(value))); const complete = selected.status?.toLowerCase() === "approved" || index < currentStage; const current = !complete && index === currentStage; return <div key={stage} className={complete ? "complete" : current ? "current" : ""}><span>{complete ? <Icon.Doc /> : index + 1}</span><div><strong>{stage}</strong><small>{complete ? "Complete" : current ? "Current handoff" : "Upcoming"}</small></div></div>; })}</div><button className="text-action" type="button" onClick={() => setSelected(selected)}>Open document details <Icon.ChevronR /></button></> : <div className="tracking-empty"><Icon.Tracking /><strong>Select a document</strong><span>Choose a tracked document to see its owner, status, and SLA details.</span></div>}</aside>
          </section>
        </main>
      </div>
