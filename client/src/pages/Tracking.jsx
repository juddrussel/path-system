...
function Avatar({ name, size = 28, color = "#7c3aed", src }) {
  const [imgFailed, setImgFailed] = useState(false);
  const resolvedSrc = fullAvatarUrl(src);
  const initials = String(name || "?").split(" ").map(w => w[0]).slice(0, 2).join("").toUpperCase();
  const showImg = resolvedSrc && !imgFailed;
  return (
    <div style={{ width: size, height: size, borderRadius: "50%", background: color, color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.4, fontWeight: 700, flexShrink: 0, overflow: "hidden" }}>
      {showImg
        ? <img
            src={resolvedSrc}
            alt={name || "User"}
...
export default function Tracking() {
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          (Array.isArray(users) ? users : []).forEach(u => {
            ...
            if (u.id != null && name) userMap[String(u.id)] = { name, avatar };
          });
        } else {
      ...
      const nameOf = (id) => (id != null && userMap[String(id)]?.name) || (id ? `User #${id}` : "—");
      const avatarOf = (id) => (id != null && userMap[String(id)]?.avatar) || null;
        ...
        if (res.ok) {
          ...
          (Array.isArray(tasks) ? tasks : []).forEach(t => {
            merged.push({
              ...
              assigned_by_avatar: avatarOf(t.assigned_by || t.created_by),
              faculty_name:     nameOf(t.faculty_id),                     // faculty who submits
              faculty_avatar:   avatarOf(t.faculty_id),
  ...
  return (
    <div className="path-tracking-shell">
      <style>{
        "@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Manrope:wght@500;600;700;800&display=swap');.path-tracking-shell{display:flex;height:calc(100vh - 60px);min-width:0;overflow:hidden;background:#f8f7ff;color:#2c2537;font-family:'DM Sans',sans-serif}.path-tracking-main{display:flex;flex:1;min-width:0;flex-direction:column;overflow:hidden}.path-tracking-content{flex:1;overflow-y:auto;padding:26px 28px 36px;background:linear-gradient(180deg,#faf9ff 0%,#f8f7ff 100%);box-sizing:border-box}@media(min-width:1100px){.path-tracking-content{padding-left:clamp(48px,5vw,84px);padding-right:clamp(48px,5vw,84px)}}.tracking-hero{display:flex;align-items:center;justify-content:space-between;gap:24px;width:100%;max-width:none;min-height:148px;margin:0 auto 20px;box-sizing:border-box;padding:29px 24px;border:1px solid #e6ddf5;border-left:2px solid #c4b5fd;border-radius:12px;background:linear-gradient(112deg,#fcfaff,#f5efff)}.tracking-hero h1{margin:9px 0 7px;color:#2c2537;font:700 31px/1.12 Manrope,sans-serif;letter-spacing:-.045em}.tracking-hero p{margin:0;color:#83778b;font:400 14px/1.4 'DM Sans',sans-serif}.tracking-kicker{display:flex;align-items:center;gap:8px;color:#8e8499;font:700 12px/1 'DM Sans',sans-serif;letter-spacing:.12em;text-transform:uppercase}.tracking-kicker i{width:6px;height:6px;border-radius:50%;background:#8b5cf6;box-shadow:0 0 0 4px #eee8ff}.tracking-primary-action{display:inline-flex;align-items:center;justify-content:center;gap:8px;height:38px;padding:0 15px;border:0;border-radius:8px;background:#7c3aed;color:#fff;box-shadow:0 8px 18px rgba(124,58,237,.16);font:700 13px/1 'DM Sans',sans-serif;cursor:pointer}.tracking-primary-action:hover{background:#6d28d9}.tracking-stat-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;width:100%;max-width:none;margin:0 auto 20px;box-sizing:border-box}.tracking-stat-card{display:flex;min-height:136px;flex-direction:column;align-items:flex-start;justify-content:flex-start;padding:23px 20px 20px;border:1px solid #e5deed;border-radius:9px;background:#fff;box-shadow:0 8px 20px rgba(57,36,93,.035);box-sizing:border-box}.tracking-stat-card>span{display:block;color:#8f8797;font:700 12px/1 'DM Sans',sans-serif;letter-spacing:.12em;text-transform:uppercase}.tracking-stat-card>strong{display:block;margin:18px 0 10px;color:#332c3e;font:700 34px/1 Manrope,sans-serif;letter-spacing:-.045em}.tracking-stat-card>small{color:#a49aa9;font:400 12px/1.3 'DM Sans',sans-serif}.tracking-stat-card small .positive{color:#3d9270;font-weight:700}.tracking-stat-card small .negative{color:#b87b55;font-weight:700}.tracking-stat-card .positive{color:#3d9270;font-weight:700}.tracking-stat-card .negative{color:#b87b55;font-weight:700}.tracking-panel{border:1px solid #e5deed!important;border-radius:12px!important;background:#fff!important;box-shadow:0 12px 30px rgba(57,36,93,.045)!important}.tracking-panel-topline{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;padding:19px 21px 16px;border-bottom:1px solid #f0edf4}.tracking-panel-topline h3{margin:6px 0 0;color:#393341;font:700 17px/1.15 Manrope,sans-serif;letter-spacing:-.025em}.tracking-panel-topline .section-kicker{color:#9d96a5;font:700 12px/1 'DM Sans',sans-serif;letter-spacing:.12em;text-transform:uppercase}.tracking-live{display:inline-flex;align-items:center;gap:7px;padding:6px 9px;border:1px solid #dcefe4;border-radius:7px;background:#f4fcf7;color:#4c9b70;font:700 12px/1 'DM Sans',sans-serif}.tracking-live .live-dot{width:6px;height:6px;border-radius:50%;background:#4c9b70}.tracking-pipeline{width:100%;max-width:none;margin:0 auto 20px;box-sizing:border-box;overflow:hidden}.tracking-pipeline-body{padding:22px 21px 24px}.pipeline-steps{display:flex;align-items:flex-start;width:100%;gap:0}.pipeline-step{display:flex;min-width:92px;flex:0 0 auto;flex-direction:column;align-items:center;gap:7px;text-align:center}.pipeline-step>span{display:grid;width:28px;height:28px;place-items:center;border:1px solid #e6dff0;border-radius:50%;background:#fff;color:#a297aa;font:700 12px/1 'DM Sans',sans-serif}.pipeline-step.complete>span{border-color:#bde5ca;background:#effaf2;color:#4c9b70}.pipeline-step.active>span{border-color:#a78bfa;background:#7c3aed;color:#fff;box-shadow:0 0 0 5px #eee8ff}.pipeline-step strong{color:#675a70;font:700 12px/1.2 Manrope,sans-serif}.pipeline-step small{color:#a49aa9;font:400 12px/1.2 'DM Sans',sans-serif}.pipeline-line{height:1px;flex:1;margin:14px 8px 0;background:#e9e2f0}.pipeline-line.complete{background:#b9dec6}.pipeline-line.active{background:linear-gradient(90deg,#b9dec6,#a78bfa)}.tracking-layout{display:grid;grid-template-columns:minmax(0,1.65fr) minmax(310px,.8fr);gap:14px;align-items:start;width:100%;max-width:none;margin:0 auto;box-sizing:border-box}.tracking-table{min-width:0;overflow:hidden}.tracking-table-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:18px;padding:20px 21px 16px;border-bottom:1px solid #f0edf4}.tracking-table-heading h3{margin:6px 0 4px;color:#393341;font:700 18px/1.15 Manrope,sans-serif;letter-spacing:-.025em}.tracking-table-heading p{margin:0;color:#9b91a3;font:400 12px/1.35 'DM Sans',sans-serif}.tracking-filter-row{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:6px}.tracking-filter-row button{padding:7px 10px;border:1px solid #e7ddf1;border-radius:999px;background:#fff;color:#776b83;font:600 12px/1 'DM Sans',sans-serif;cursor:pointer}.tracking-filter-row button.active{border-color:#7c3aed;background:#7c3aed;color:#fff;box-shadow:0 5px 12px rgba(124,58,237,.14)}.tracking-table-head{display:grid;grid-template-columns:minmax(250px,1.6fr) minmax(135px,.9fr) minmax(135px,.9fr) minmax(150px,1fr) 120px;gap:16px;padding:11px 18px;background:#fcfaff;color:#93879b;font:800 11px/1 'DM Sans',sans-serif;letter-spacing:.09em;text-transform:uppercase}.tracking-records{background:#fff}.tracking-record{display:grid;grid-template-columns:minmax(250px,1.6fr) minmax(135px,.9fr) minmax(135px,.9fr) minmax(150px,1fr) 120px;gap:16px;align-items:center;width:100%;min-height:78px;padding:13px 18px;border:0;border-bottom:1px solid #f0edf4;background:#fff;text-align:left;cursor:pointer}.tracking-record:hover,.tracking-record.selected{background:#fbf9ff;box-shadow:inset 3px 0 #8b5cf6}.tracking-document,.tracking-owner,.tracking-assignee,.tracking-stage{display:flex;align-items:center;min-width:0;gap:10px}.tracking-document>div{display:flex;min-width:0;flex-direction:column;align-items:flex-start;gap:4px}.tracking-assignee{gap:8px}.tracking-assignee>span{overflow:hidden;color:#685b72;font:500 12px/1.2 'DM Sans',sans-serif;text-overflow:ellipsis;white-space:nowrap}.tracking-stage{flex-direction:column;align-items:flex-start;gap:4px}.tracking-record-id{color:#7c3aed;font:700 12px/1 'DM Sans',sans-serif;letter-spacing:.05em}.tracking-document strong{display:block;max-width:100%;overflow:hidden;color:#40364b;font:700 13px/1.25 Manrope,sans-serif;text-overflow:ellipsis;white-space:nowrap}.tracking-document>div>span:last-child,.tracking-stage>span:last-child{color:#a49aa9;font:400 12px/1.25 'DM Sans',sans-serif}.tracking-owner>span:last-child{overflow:hidden;color:#685b72;font:500 12px/1.2 'DM Sans',sans-serif;text-overflow:ellipsis;white-space:nowrap}.tracking-sla{display:inline-flex;align-items:center;gap:7px;color:#4c9b70;font:700 12px/1.2 'DM Sans',sans-serif}.tracking-sla i{width:6px;height:6px;border-radius:50%;background:currentColor}.tracking-sla.risk{color:#bd7c2b}.tracking-sla.closed{color:#98909e}.tracking-detail{display:flex;min-height:550px;flex-direction:column;padding:20px 21px 22px}.tracking-detail .tracking-panel-topline{padding:0 0 16px}.tracking-detail-title{display:flex;align-items:center;gap:11px;padding:18px 0 18px;border-bottom:1px solid #f0edf4}.tracking-detail-title>div{display:flex;min-width:0;flex-direction:column;gap:4px}.tracking-detail-title strong{display:block;overflow:hidden;color:#40364b;font:700 15px/1.2 Manrope,sans-serif;letter-spacing:-.01em;text-overflow:ellipsis;white-space:nowrap}.tracking-detail-title>div>span{color:#a49aa9;font:500 12px/1.3 'DM Sans',sans-serif}.tracking-detail-avatar{display:grid;width:35px;height:35px;flex:none;place-items:center;border-radius:11px;background:#f0eaff;color:#7c3aed;font:800 12px/1 'DM Sans',sans-serif;overflow:hidden}.tracking-detail-status-card{display:flex;flex-direction:column;gap:8px;margin-top:18px;padding:16px;border:1px solid #e7e0ee;border-radius:8px;background:#fbfaff}.tracking-detail-status-card .section-kicker{color:#9d96a5;font:700 12px/1 'DM Sans',sans-serif;letter-spacing:.12em;text-transform:uppercase}.tracking-detail-status-card strong{color:#7040ba;font:700 20px/1 Manrope,sans-serif}.tracking-detail-status-card>span:last-child{color:#8f8499;font:400 12px/1.4 'DM Sans',sans-serif}.tracking-mini-timeline{position:relative;display:flex;flex-direction:column;gap:0;padding:22px 0 26px}.tracking-mini-timeline:before{position:absolute;top:30px;bottom:32px;left:10px;width:1px;background:#e6e0ed;content:''}.tracking-mini-timeline>div{position:relative;display:flex;align-items:flex-start;gap:11px;min-height:49px}.tracking-mini-timeline>div>span{position:relative;z-index:1;display:grid;width:20px;height:20px;flex:none;place-items:center;border:1px solid #e4dcef;border-radius:50%;background:#fff;color:#a297aa;font:700 12px/1 'DM Sans',sans-serif;box-shadow:0 0 0 3px #fff}.tracking-mini-timeline>div.complete>span{border-color:#bde5ca;background:#effaf2;color:#4c9b70}.tracking-mini-timeline>div.current>span{border-color:#a78bfa;background:#7c3aed;color:#fff;box-shadow:0 0 0 3px #eee8ff}.tracking-mini-timeline>div.complete strong{color:#675a70}.tracking-mini-timeline>div.current strong{color:#6d35b9}.tracking-mini-timeline>div>div{display:flex;flex-direction:column;gap:4px;padding-top:2px}.tracking-mini-timeline strong{color:#675a70;font:700 12px/1.2 Manrope,sans-serif}.tracking-mini-timeline small{color:#a49aa9;font:400 12px/1.2 'DM Sans',sans-serif}.tracking-detail .text-action{display:inline-flex;align-items:center;gap:7px;margin-top:auto;padding:10px 0 0;border:0;background:none;color:#7c3aed;font:700 12px/1.2 'DM Sans',sans-serif;cursor:pointer}.tracking-pagination{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:14px 18px;border-top:1px solid #f0edf4;color:#a49aa9;font:400 12px/1 'DM Sans',sans-serif}.tracking-page-buttons{display:flex;gap:6px}.tracking-page-buttons button{display:grid;width:29px;height:29px;place-items:center;border:1px solid #e7ddf1;border-radius:7px;background:#fff;color:#887c92;font:600 12px/1 'DM Sans',sans-serif;cursor:pointer}.tracking-page-buttons button.active{border-color:#7c3aed;background:#7c3aed;color:#fff}.tracking-page-buttons button:disabled{background:#faf9fc;color:#d1cbd7;cursor:default}.tracking-empty{display:flex;align-items:center;flex-direction:column;gap:6px;padding:48px 20px;color:#a49aa9;text-align:center}.tracking-detail>.tracking-empty{flex:1;justify-content:center;padding:32px 18px}.tracking-empty strong{color:#675a70;font:700 13px/1.2 Manrope,sans-serif}.tracking-empty span{font:400 12px/1.35 'DM Sans',sans-serif}@media(max-width:1100px){.tracking-layout{grid-template-columns:1fr}.tracking-detail{order:-1}.tracking-stat-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:760px){.path-tracking-content{padding:17px 14px 28px}.tracking-hero{align-items:flex-start;flex-direction:column;gap:18px;min-height:0;padding:22px 18px}.tracking-hero h1{font-size: 27px}.tracking-hero .tracking-primary-action{width:100%}.tracking-stat-grid{gap:8px}.tracking-stat-card{min-height:112px;padding:18px 15px}.tracking-stat-card>strong{margin:14px 0 8px;font-size: 27px}.tracking-pipeline-body{overflow-x:auto}.pipeline-steps{min-width:560px}.tracking-table-heading{align-items:stretch;flex-direction:column;gap:13px}.tracking-filter-row{justify-content:flex-start}.tracking-table-head{display:none}.tracking-record{grid-template-columns:minmax(0,1fr) auto;gap:8px;padding:14px}.tracking-owner,.tracking-assignee{display:none}.tracking-stage{grid-column:2;grid-row:1;align-items:flex-end}.tracking-sla{grid-column:1/-1;padding-left:0}.tracking-pagination{align-items:flex-start;flex-direction:column}.tracking-page-buttons{align-self:flex-end}}"
      }</style>
        ...
        <main className="path-tracking-content">
          ...
          <section className="tracking-layout">
            <article className="tracking-table tracking-panel">
              ...
              <div className="tracking-table-head"><span>Document</span><span>Assigned to</span><span>Review by</span><span>Stage</span><span>SLA</span></div>
              <div className="tracking-records">{loading ? <div className="tracking-empty"><Icon.Refresh /><strong>Loading tracking records</strong><span>Syncing the latest document handoffs.</span></div> : paginated.length === 0 ? <div className="tracking-empty"><Icon.Tracking /><strong>No documents match these filters</strong><span>Try another search or lifecycle status.</span></div> : paginated.map((doc) => <button key={doc.id || doc.document_id} type="button" className={"tracking-record " + (selected?.id === doc.id ? "selected" : "")} onClick={() => setSelected(doc)}><div className="tracking-document"><div><span className="tracking-record-id">{doc.document_id}</span><strong>{doc.title}</strong><span>{doc.department}{doc.source_type ? " · " + doc.source_type : ""}</span></div></div><div className="tracking-assignee"><Avatar name={doc.faculty_name || doc.current_handler || "Faculty"} src={doc.faculty_avatar} size={28} color="#7c3aed" /><span>{doc.faculty_name || doc.current_handler || "Faculty"}</span></div><div className="tracking-assignee"><Avatar name={doc.assigned_by_name || doc.assigned_by || "Program Chair"} src={doc.assigned_by_avatar} size={28} color="#a78bfa" /><span>{doc.assigned_by_name || doc.assigned_by || "Program Chair"}</span></div><div className="tracking-stage"><StatusBadge status={doc.status} /><span>{doc.stage || "Current handoff"}</span></div><div className={"tracking-sla " + (["returned", "rejected"].includes(doc.status?.toLowerCase()) ? "risk" : ["approved", "archived"].includes(doc.status?.toLowerCase()) ? "closed" : "")}><i />{["returned", "rejected"].includes(doc.status?.toLowerCase()) ? "Needs attention" : ["approved", "archived"].includes(doc.status?.toLowerCase()) ? "Closed" : "On track"}</div></button>)}</div>
            ...
            <aside className="tracking-detail tracking-panel"><div className="tracking-panel-topline"><div><div className="section-kicker">Selected record</div><h3>Current handoff</h3></div><button className="icon-button compact" type="button" onClick={() => selected && setSelected(selected)} aria-label="Refresh selected document"><Icon.Refresh /></button></div>{selected ? <><div className="tracking-detail-title"><span className="tracking-detail-avatar">{fullAvatarUrl(selected.faculty_avatar || selected.assigned_by_avatar) ? <img src={fullAvatarUrl(selected.faculty_avatar || selected.assigned_by_avatar)} alt={selected.submitted_by || selected.current_handler || "User"} onError={(e) => { e.currentTarget.style.display = "none"; e.currentTarget.nextSibling.style.display = "block"; }} style={{ width: "100%", height: "100%", objectFit: "cover", borderRadius: 11 }} /> : null}<span style={{ display: fullAvatarUrl(selected.faculty_avatar || selected.assigned_by_avatar) ? "none" : "block" }}>{String(selected.submitted_by || selected.current_handler || "PATH").split(/\s+/).map((word) => word[0]).join("").slice(0, 2).toUpperCase()}</span></span><div><strong>{selected.title}</strong><span>{selected.document_id} · {selected.current_handler || "Program Chair"}</span></div></div><div className="tracking-detail-status-card"><span className="section-kicker">Current status</span><strong>{selected.status}</strong><span>{selected.status?.toLowerCase() === "approved" ? "Closed on time" : selected.stage || "Current handoff"}{selected.submitted_at ? " · due " + new Date(selected.submitted_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : ""}</span></div><div className="tracking-mini-timeline">{["Submitted", "Screening", "Review", "Decision"].map((stage, index) => { const currentStage = Math.max(0, ["submitted", "screening", "review", "decision"].findIndex((value) => String(selected.stage || selected.status || "").toLowerCase().includes(value))); const complete = selected.status?.toLowerCase() === "approved" || index < currentStage; const current = !complete && index === currentStage; return <div key={stage} className={complete ? "complete" : current ? "current" : ""}><span>{complete ? <Icon.Doc /> : index + 1}</span><div><strong>{stage}</strong><small>{complete ? "Complete" : current ? "Current handoff" : "Upcoming"}</small></div></div>; })}</div><button className="text-action" type="button" onClick={() => setSelected(selected)}>Open document details <Icon.ChevronR /></button></> : <div className="tracking-empty"><Icon.Tracking /><strong>Select a document</strong><span>Choose a tracked document to see its owner, status, and SLA details.</span></div>}</aside>
          </section>
        </main>
      </div>
