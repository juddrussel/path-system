...
function FacultySubmissionsWorkspace({
  ...
  return (
    <div
      className={`faculty-submissions-shell ${embedded ? "is-embedded" : ""}`}
    >
      <style>{`
      ...
      {embedded && (
        <style>{`
          .faculty-submissions-shell.is-embedded{display:block;min-height:0;background:transparent}
                ...
                visible.map((row) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="faculty-submission-submitter">
                        <i>
                          {initialsFor(
                            row.full_name || row.submitter_name || "Faculty",
                          )}
                        </i>
                        <span>
                          {row.full_name || row.submitter_name || "You"}
                        </span>
                      </span>
...
export default function Forms() {
  ...
  // Unfiltered list (all types & statuses) used by the Form Catalog tab,
  // so it can show every form that exists — not just the Active ones the
  // submission wizard is allowed to offer.
  ...
  // ── Form Catalog tab: every Form-type category, Active or not ────────────
  const catalogForms = allCategories
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      const formCategories = allCats.filter(
  ...
  return (
    <div
      className={`path-forms-shell ${isProgramChair && activeTab === "review" ? "path-forms-queue-active" : ""} ${!isProgramChair && activeTab === "submit" ? "path-forms-start-active" : ""} ${!isProgramChair && activeTab === "history" ? "path-forms-register-active" : ""} ${activeTab === "catalog" ? "path-forms-catalog-active" : ""}`}
      style={{
        display: "flex",
        minHeight: "100vh",
        fontFamily: "'DM Sans', sans-serif",
    ...
    >
      ...
      <style>{`
        ...
        .path-forms-catalog-active .path-forms-default-header,.path-forms-catalog-active .path-forms-default-stats{display:none!important}
        .path-forms-catalog-active .path-forms-hero{align-items:center;min-height:148px}
      ...
      >
        ...
        >
          ...
          {/* ── FACULTY: MY SUBMISSIONS PATH REGISTER ── */}
          {activeTab === "history" && !isProgramChair && (
          ...
          {/* ── FORM CATALOG (all roles) ── */}
          {activeTab === "catalog" && (
            <>
              <section className="path-forms-hero">
                <div>
                  <div className="path-forms-kicker">
                    <i /> Form catalog · reference library
                  </div>
                  <h1>Form catalog</h1>
                  <p>
                    Every form type configured for the department, what it
                    requires, and whether it's currently open for
                    submission.
                  </p>
                </div>
                <div className="path-forms-health">
                  ...
                  <span>
                    <strong>{catalogForms.length} form types</strong>
                    <small>
                      {catalogForms.filter((c) => c.status === "Active").length}{" "}
                      currently active
                    </small>
                  </span>
                </div>
              ...
              <section className="path-forms-catalog">
                <div className="path-forms-catalog-toolbar">
                  <div className="path-forms-catalog-search">
                    <Icon.Search />
                    <input
                      type="text"
                ...
                {categoriesLoading ? (
                  <div className="path-forms-catalog-empty">
                    <strong>Loading form catalog…</strong>
                  </div>
                ) : catalogForms.length ? (
                  <div className="path-forms-catalog-grid">
                    {catalogForms.map((cat) => {
                      const fieldCount = (cat.formFields || []).length;
                      return (
                        <article key={cat.id} className="path-forms-catalog-card">
                          <div className="path-forms-catalog-card-head">
                            <span
                              className={`path-forms-catalog-status ${
                            ...
                            {cat.code && (
                              <span className="path-forms-catalog-code">
                                {cat.code}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="path-forms-catalog-meta">
                            <span>
                              {fieldCount} field{fieldCount === 1 ? "" : "s"}
                            </span>
                            {!isProgramChair && cat.status === "Active" && (
                              <button
                                type="button"
                                onClick={() => {
                                  setWizardFormType(cat.name);
                                  setActiveTab("submit");
                                }}
                              >
                                Use this form →
          ...
          {/* ── LEGACY REVIEW TABLE (Program Chair only; review queue renders above) ── */}
          {isProgramChair && activeTab === "review" && (
            <div
              style={{
                display: "none",
