...
function PathAssignedWorkspace({
  ...
  const exportCsv = () => {
    ...
    link.href = url;
    link.download = "path-assigned-tasks.csv";
    link.click();
  ...
  return (
    <div className="path-assigned-shell" style={{ display: "flex", height: "100vh", overflow: "hidden" }}>
      <style>{`
        ...
        .path-assigned-controls{display:flex;align-items:center;gap:8px;margin-bottom:12px}.path-assigned-search{display:flex;align-items:center;gap:8px;flex:1;min-width:210px;padding:0 11px;border:1px solid #e2dbe9;border-radius:8px;background:#fff;color:#9a8fa3}.path-assigned-search input{width:100%;height:36px;border:0;outline:0;background:transparent;color:#4c3e57;font-size: 12px}.path-assigned-filter{display:flex;align-items:center;gap:6px;height:36px;max-width:152px;padding:0 8px;border:1px solid #e2dbe9;border-radius:8px;background:#fff;color:#786b83}.path-assigned-filter span{color:#93869e;font-size: 11px;font-weight:800;white-space:nowrap}.path-assigned-filter select,.path-assigned-date{min-width:0;flex:1;border:0;outline:0;background:transparent;color:#665574;font-size: 12px}.path-assigned-date{height:36px;flex:none;max-width:142px;padding:0 8px;border:1px solid #e2dbe9;border-radius:8px;background:#fff}
        ...
        <div className="path-assigned-body">
          <div className="path-assigned-inner">
            <header className="path-assigned-hero">
              <div>
                <span className="path-assigned-eyebrow">
                  <i /> YOUR WORKSPACE · UPDATED JUST NOW
                </span>
                <h1>Assigned</h1>
                <p>Manage and approve tasks assigned specifically to you.</p>
                <div className="path-assigned-focus">
                  <Icon.Clock />
                  <span>
                    <strong>{overdueCount} decisions need attention</strong>
                    <small>
                      Start with the oldest handoff to protect your review
                      window.
                    </small>
                  </span>
                </div>
              </div>
              <div className="path-assigned-actions">
                <button type="button" onClick={onBack}>
                  ← Back to task desk
                </button>
                <button type="button" className="violet" onClick={exportCsv}>
            ...
            <section className="path-assigned-metrics">
              {[
                ...
                },
                {
                  label: "Due today",
                  value: dueTodayCount,
                  note: "Time-sensitive handoffs",
                  icon: <Icon.Clock />,
                },
                {
                  label: "Needs attention",
                  value: overdueCount,
                  note: "Past review target",
                  icon: "!",
                  tone: "danger",
                },
                {
                  ...
                  value: readyCount,
                  note: "Awaiting your action",
                  icon: "◷",
                },
              ].map((metric) => (
                <article
                  className={`path-assigned-metric ${metric.tone || ""}`}
                  key={metric.label}
                >
                  <div>
                    <small>{metric.label}</small>
            ...
            <section
              ...
              aria-label="Assigned task controls"
            >
            ...
            <section className="path-assigned-workspace">
              <aside className="path-assigned-feed">
                <header className="path-assigned-feed-head">
                  <div>
              ...
              <div className="path-assigned-detail">
                {selected ? (
                  <article className="path-assigned-detail-card">
                    <header>
                      <div>
                        <small>
                          Decision brief · {selected.tracking_id || selected.id}
                        </small>
                        <h2>{selected.title || "Untitled task"}</h2>
                      </div>
                      <span className="path-assigned-status">
                    ...
                    {detailLoading ? (
                      ...
                      <>
                        <div className="path-assigned-detail-grid">
                          <span>
                            Tracking ID
                            <strong>
                          ...
                          <span>
                            Document
                            <strong>
                              {selected.doc_type || "Workflow task"}
                            </strong>
                          </span>
                          <span>
                            Priority
                            <strong>{selected.priority || "Medium"}</strong>
                          </span>
                          <span>
...
export default function MyTasks() {
  ...
  const openAssignedView = () => {
    ...
    next.set("role", "faculty");
    setSearchParams(next);
  };
  const closeAssignedView = () => {
    const next = new URLSearchParams(searchParams);
    next.delete("view");
    next.delete("role");
    setSearchParams(next);
